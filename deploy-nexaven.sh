...
# NEXAVEN ROOT DEPLOYMENT SCRIPT
# Bu script root dizininden çalışır ve tüm sistemi kurar
...
echo -e "${PURPLE}🚀 NEXAVEN ROOT DEPLOYMENT BAŞLATIYOR...${NC}"
echo -e "${BLUE}================================================${NC}"
...
if [ ! -d "nexaven-website" ]; then
    echo -e "${RED}❌ nexaven-website dizini bulunamadı! Doğru dizinde olduğunuzdan emin olun.${NC}"
    exit 1
fi

if [ ! -f "nexaven-website/docker-compose.yml" ]; then
...
# 3. NEXAVEN-WEBSITE DİZİNİNE GEÇ
echo -e "${BLUE}📁 3/10 - nexaven-website dizinine geçiliyor...${NC}"
cd nexaven-website
echo -e "${GREEN}✅ Dizin değiştirildi: $(pwd)${NC}"

# 4. ESKİ CONTAINER'LARI DURDUR
echo -e "${BLUE}🛑 4/10 - Eski container'lar durduruluyor...${NC}"
$DOCKER_COMPOSE_CMD down --remove-orphans || true
docker system prune -f || true
echo -e "${GREEN}✅ Eski container'lar temizlendi${NC}"

# 5. NGINX CONFIG GÜNCELLEMESİ
echo -e "${BLUE}🔧 5/10 - Nginx konfigürasyonu güncelleniyor...${NC}"
...
# 6. CAR DATA, ECU DATA VE LOGO KONTROLÜ
echo -e "${BLUE}📁 6/10 - Car data, ECU data ve logo dizinleri kontrol ediliyor...${NC}"
if [ -d "car_data" ]; then
...
# 7. DOCKER COMPOSE KONTROLÜ
echo -e "${BLUE}🐳 7/10 - Docker Compose konfigürasyonu kontrol ediliyor...${NC}"
...
# 8. FRONTEND BUILD
echo -e "${BLUE}🏗️  8/10 - Frontend build ediliyor...${NC}"
...
# 9. SSL DİZİNLERİ HAZIRLA
echo -e "${BLUE}🔐 9/10 - SSL dizinleri hazırlanıyor...${NC}"
mkdir -p nginx/ssl
echo -e "${GREEN}✅ SSL dizinleri hazır${NC}"

# 10. CONTAINER'LARI BAŞLAT
echo -e "${BLUE}🚀 10/10 - Container'lar başlatılıyor...${NC}"
...
echo ""
echo -e "${BLUE}🔧 Yararlı Komutlar (nexaven-website dizininde):${NC}"
